...
import           Text.ExprParser.ParseRownum (ExprParam(..), runRNT)
import           Text.Parsec
...
projectionParam ns0 (ParamsL ns vss)
  | ns0 == mempty = ParamsL [] [[]]
  | otherwise = uncurry ParamsL
    $ second (nub . transpose) $ unzip $ filter ((`elem` ns0) . fst)
    $ zip ns $ transpose vss
...
parseL = (\res -> if null res then mempty else foldr1 (<+>) res) <$> sumList
  -- парсится сумма произведений (например, A*B+A*D)
  where
    sumList = parseProd `sepBy` char '+'
    parseProd = fmap mconcat $ parseL1 `sepBy` oneOf ";*"
    readLex = readTuple term
    parseL1
      = readLex >>= \ns -> char '?' >> readTuple readLex >>= return
      . ParamsL (map trim ns) . map (map trim . take (length ns))
    trim = T.pack . trim' . trim'
      where trim' = reverse . dropWhile isSpace

readTuple :: ParserL a -> ParserL [a]
readTuple p  = noSpaces
  $ char '[' >> (noSpaces p `sepBy` char ',') >>= \r -> char ']' >> return r
  where
...
getParamsL s = p
  { pVals = concatMap (\p' -> concatMap (applyParam p') $ pVals p')
    $ paramsToList p }
  where
    p = case runParser parseL 0 "Params List Parser" s of
      Left err -> error $ unlines
        [ "Error in getParams String to parse: "
        , T.unpack s
        , "", "Error: ", show err]
      Right xs -> xs
...
class ApplyParam t where
  ...
  appP ::
    t
    -- ^ значение, к которому применяется параметр
    -> ExprParam
    -- ^ дополнительные параметры - текущий номер параметра и общее количество параметров (obsolete?)
    -> (T.Text->T.Text, ([T.Text], [T.Text]))
    -- ^ (функция, помещающая название параметра в скобки; пара названия - значения)
    -> t
...
instance ApplyParam T.Text where
  appP t n (fb, (ns,vs)) =
    runRNT n $ fst $ head $ dropWhile (uncurry (/=)) $ zip zs (tail zs)
    where
...
instance ApplyParam Name where
  appP (Name n nsp px) k
    = (Name <$> appP n k <*> appP nsp k <*> appP px k)
    . first (const $ \t -> mconcat ["p_",t,"_p"])
...
instance ApplyParam Event where
  ...
  appP (EventBeginDoctype t mid) k =
    EventBeginDoctype <$> appP t k <*> appP mid k
  appP EventEndDoctype           _ = pure EventEndDoctype
  appP (EventInstruction i)      k = EventInstruction  <$> appP i k
  appP (EventBeginElement n xs)  k =
    EventBeginElement <$> appP n k <*> appP xs  k
